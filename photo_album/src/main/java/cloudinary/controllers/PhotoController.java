...
public class PhotoController {
    ...
    public String uploadPhoto(@ModelAttribute PhotoUpload photoUpload, BindingResult result, ModelMap model) throws IOException {
        ...
        Map options = ObjectUtils.asMap("transformation", new Transformation().width(500).height(500).crop("limit"),
            "tags", "simple");
        if (photoUpload.getFile() != null && !photoUpload.getFile().isEmpty()) {
            //uploadResult = Singleton.getCloudinary().uploader().upload(photoUpload.getFile(), options);
            
            uploadResult = Singleton.getCloudinary().uploader().upload(photoUpload.getFile().getBytes(), options);
